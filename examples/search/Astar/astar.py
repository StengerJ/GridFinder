...
def h(coord: tuple[int, int], goals: list[tuple[int, int]]) -> int:
    """
...
def constructPath(
    ...
    """
    Reconstructs the path from the start coordinate to the goal coordinate using the parents and actions_to dictionaries.
        - parents: A dictionary mapping each coordinate to its parent coordinate in the search tree.
        - actions_to: A dictionary mapping each coordinate to the action taken to reach it from its parent.
    ...
    current = goal
    while current != start:
        actions.append(actions_to[current])
...
def getGoal(env) -> list[tuple[int, int]]:
    """
    Extracts the goal positions from the environment. 
